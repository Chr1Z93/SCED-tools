...
# This will instruct the sorter to sort by internal CardID instead of by card name (not ArkhamDB ID)
SORT_BY_ID = True
...
def rebuild_deck_data(
    ...
    for filename_stem in contained_objects:
        ...
        card_custom_deck = card_json.get("CustomDeck", {})
    ...
    if not PRESERVE_CARD_ORDER:
        if SORT_BY_ID:
            ...
            temp_card_list.sort(key=lambda x: x[0], reverse=True)

    data["ContainedObjects_order"] = [x[0] for x in temp_card_list]
    data["DeckIDs"] = [x[1] for x in temp_card_list]
...
def clean_deck(
    ...
    2. Identifies the files (ContainedObjects) that need to be discarded.
    ...
    for obj, deck_id in zip(contained_objects, deck_ids):
...
def cleanup_everything(root_path: Path):
    # Global Shallow Fix (Fixes wrong card ids first)
    global_shallow_fix(root_path)

    # Process the root folder itself
    process_folder_for_cleanup(root_path)
...
if __name__ == "__main__":
    if ROOT_FOLDER_PATH.is_dir():
        ...
        print(f"Can't find {ROOT_FOLDER_PATH}")
